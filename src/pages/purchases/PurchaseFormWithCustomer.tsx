...
export default function PurchaseFormWithCustomer({ initial, customers, onSubmit, onCancel, loading }: Props) {
  const today = new Date().toISOString().split('T')[0]
  const [form, setForm] = useState({
    u_customer: initial?.u_customer ?? '',
  ...
  return (
    <form onSubmit={e => { e.preventDefault(); onSubmit(form) }} className="space-y-4">
      <div>
        ...
        <select className="input-field" required value={form.u_customer} onChange={e => set('u_customer', e.target.value)}>
          <option value="">Select customer...</option>
          {customers.map(c => (
            <option key={c.sys_id} value={c.sys_id}>{c.u_name}{c.u_company ? ` — ${c.u_company}` : ''}</option>
          ))}
        </select>
      </div>
      <div>
        <label className="label">Product / Service *</label>
        <input className="input-field" required value={form.u_product_name} onChange={e => set('u_product_name', e.target.value)} placeholder="e.g. Consulting Package" />
      </div>
      ...
      <div className="flex gap-3 pt-2">
        ...
        <button type="submit" className="btn-primary flex-1 justify-center" disabled={loading}>
          {loading ? <><Loader2 className="w-4 h-4 animate-spin" /> Saving...</> : 'Save Order'}
        </button>
      </div>
    </form>
  )
